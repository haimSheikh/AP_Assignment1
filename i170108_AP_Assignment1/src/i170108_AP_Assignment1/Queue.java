...
public class Queue {
	...
	public Queue(int size) {
		...
		removePointer = 0;
		sizeofQueue = 0;
		array = new int[size + 1]; // because addPointer and removePointer cannot point at same index,so in order
									// to create a buffer i.e space of one box or index between them.
	}
	...
	public void enqueue(int number) throws Exception {
		...
		// if addPointer point at last position so we have to check whether
		// removePointer points at 0 position or not before adding an element at the
		// last position,to maintain a space between the pointers.
